...
my $PBX_CONFIG_FILE_PATH = '/usr/local/nagios/etc/avaya/';
...
sub _initialize {
#=============================================================
    my ($self, $nodename, $debug_param, $PBX_CONFIG_FILE_PATH_PARAM) = @_;

    if ( $debug_param ) {
        $DEBUG = $debug_param;
    }
	if ( $PBX_CONFIG_FILE_PATH_PARAM && $PBX_CONFIG_FILE_PATH_PARAM ne '') {
        $PBX_CONFIG_FILE_PATH = $PBX_CONFIG_FILE_PATH_PARAM;
    }

    print "getting connection parameters for $nodename\n" if $DEBUG;

    my $config = XMLin( "$PBX_CONFIG_FILE_PATH" . PBX_CONFIG_FILE );
	...
	elsif ( defined $config->{'pbx'}->{'name'} && $config->{'pbx'}->{'name'} eq  $nodename) {
        ${$self->{'NODENAME'}}        = $nodename;
        ${$self->{'HOSTNAME'}}        = $config->{'pbx'}->{'hostname'};
        ${$self->{'PORT'}}            = $config->{'pbx'}->{'port'};
        ${$self->{'USERNAME'}}        = $config->{'pbx'}->{'login'};
        ${$self->{'PASSWORD'}}        = $config->{'pbx'}->{'password'};
        ${$self->{'CONNECTION_TYPE'}} = $config->{'pbx'}->{'connection_type'};
        ${$self->{'ATDT'}}            = $config->{'pbx'}->{'atdt'};
